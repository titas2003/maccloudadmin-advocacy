...
export default function Appointments() {
  ...
  const [expandedAdv, setExpandedAdv] = useState(null); // Track which advocate's appointments are expanded
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    try {
      ...
      if (statusFilter !== 'All') {
        ...
        if (dbStatus === 'scheduled') dbStatus = 'accepted';
  ...
  useEffect(() => { setPage(1); setExpandedAdv(null); }, [statusFilter]);
  ...
  // Filter appointments client-side by search
  const filteredAppointments = appointments.filter(a => {
    if (!search) return true;
    const q = search.toLowerCase();
    return a.advocateId?.name?.toLowerCase().includes(q) || 
           a.clientId?.name?.toLowerCase().includes(q) ||
           a.advocateId?.advId?.toLowerCase().includes(q);
  });

  // Group appointments by advocate
  const groupedAppointments = filteredAppointments.reduce((acc, appt) => {
    const advId = appt.advocateId?._id || 'unknown';
    if (!acc[advId]) {
      acc[advId] = {
        advocate: appt.advocateId,
        appointments: []
      };
    }
    acc[advId].appointments.push(appt);
    return acc;
  }, {});
    ...
    <Layout>
      ...
      {statusModal && (
        <div className="fixed inset-0 z-[150] bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm p-6 shadow-2xl">
            ...
            <h3 className="text-lg font-bold text-slate-800 mb-1">Override Status</h3>
            ...
            <div className="flex gap-3">
              <button onClick={() => setStatusModal(null)} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-slate-600 text-sm font-semibold hover:bg-slate-50 transition-all">Cancel</button>
              <button onClick={handleUpdateStatus} disabled={actionLoading} className="flex-1 py-2.5 rounded-xl bg-blue-600 text-white text-sm font-bold hover:bg-blue-700 transition-all disabled:opacity-40">
          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-lg bg-white z-[110] shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="flex items-center justify-between">
                <StatusBadge status={selected.status === 'accepted' ? 'Scheduled' : (selected.status?.charAt(0).toUpperCase() + selected.status?.slice(1))} />
                <div className="flex gap-2">
                  {selected.status !== 'cancelled' && (
                    <button onClick={() => setStatusModal({ appointment: selected, newStatus: 'cancelled' })} className="px-3 py-1.5 text-xs font-bold bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-all">
                      Cancel Appt
                    </button>
                  )}
                  {selected.status !== 'completed' && (
                    <button onClick={() => setStatusModal({ appointment: selected, newStatus: 'completed' })} className="px-3 py-1.5 text-xs font-bold bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-all">
                      Mark Completed
                    </button>
                  )}
                </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-7">
        {[
          ...
          { label: 'Scheduled',  value: appointments.filter(a => a.status === 'accepted').length,  color: 'bg-indigo-50', icon: <CalendarDays size={18} className="text-indigo-500" /> },
        ...
        <div className="px-5 py-4 border-b border-slate-100 flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="relative w-full md:max-w-xs">
            ...
            <input
              ...
              placeholder="Search advocate or client..."
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {Object.values(groupedAppointments).map(group => {
                  const advId = group.advocate?._id || 'unknown';
                  const isExpanded = expandedAdv === advId;

                  return (
                    <React.Fragment key={advId}>
                      {/* Advocate Header Row */}
                      <tr 
                        className="bg-slate-100/50 cursor-pointer hover:bg-slate-100 transition-colors"
                        onClick={() => toggleExpand(advId)}
                      >
                        <td colSpan="4" className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <ChevronRight size={18} className={`text-slate-400 transition-transform ${isExpanded ? 'rotate-90' : ''}`} />
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-slate-800 text-sm">{group.advocate?.name || 'Unknown Advocate'}</span>
                              {group.advocate?.advId && (
                      ...
                      {/* Expanded Appointments Rows */}
                      {isExpanded && group.appointments.map(appt => (
                        <tr key={appt._id} className="hover:bg-slate-50/50 transition-colors bg-white">
                          <td className="px-5 py-3.5 pl-12 border-l-[3px] border-l-blue-400">
                            <p className="font-bold text-slate-700 text-sm">{appt.clientId?.name || 'Unknown Client'}</p>
                          ...
                          <td className="px-5 py-3.5"><StatusBadge status={appt.status === 'accepted' ? 'Scheduled' : appt.status.charAt(0).toUpperCase() + appt.status.slice(1)} /></td>
                          <td className="px-5 py-3.5">
                            <div className="flex items-center justify-end gap-1.5">
                              <button onClick={(e) => { e.stopPropagation(); openDetail(appt); }}
